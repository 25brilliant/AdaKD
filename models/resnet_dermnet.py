...
__all__ = ['resnet18_dermnet', 'resnet18_dermnet_aux', 'resnet34_dermnet',
           'resnet34_dermnet_aux', 'resnet50_dermnet', 'resnet50_dermnet_aux']
...
class ResNet(nn.Module):
    ...
    def forward(self, x, is_feat=False, teacher_feature=None):
        ...
        if is_feat:
            return [out1, out2, out3, out4], [asj1, asj2, asj3, asj4, x, a, b, c, d, out]
        else:
            return x
...
class Auxiliary_Classifier(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        norm_layer = nn.BatchNorm2d  # different
        downsample = None
        previous_dilation = self.dilation
        if dilate:
            self.dilation *= stride
...
class ResNet_Auxiliary(nn.Module):
    ...
    def forward(self, x, grad=False, teacher_feature=None):
        if grad is False:
            feats, logit = self.backbone(x, is_feat=True, teacher_feature=None)
            for i in range(len(feats)):
                feats[i] = feats[i].detach()
        else:
            feats, logit = self.backbone(x, is_feat=True, teacher_feature=teacher_feature)

        ss_logits = self.auxiliary_classifier(feats)

        return logit, ss_logits, feats, self.logsigma1
